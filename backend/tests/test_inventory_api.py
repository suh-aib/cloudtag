...
def test_dashboard_stats(test_db):
    ...
    assert data["tagging_required"] == 3

def test_dashboard_stats_provider_filter(test_db):
    response = client.get("/api/inventory/dashboard/stats?provider=AZURE")
    assert response.status_code == 200
    data = response.json()
...
def test_inventory_accounts_azure(test_db):
    ...
    assert data[0]["account_name"] == "Sub1"
    assert data[0]["resource_count"] == 2
    assert data[0]["group_count"] == 1
    
    assert data[1]["account_name"] == "Sub2"
    assert data[1]["resource_count"] == 1
    assert data[1]["group_count"] == 1
...
def test_inventory_accounts_aws(test_db):
    ...
    data = response.json()
    assert len(data) == 1
    assert data[0]["account_name"] == "AwsAcc1"
...
def test_aws_regions(test_db):
    response = client.get("/api/inventory/aws/accounts/aws-1/regions")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]["name"] == "us-east-1"
    assert data[0]["resource_count"] == 1
    assert data[0]["types_count"] == 1

def test_aws_regions_invalid_account(test_db):
    response = client.get("/api/inventory/aws/accounts/invalid-aws/regions")
    assert response.status_code == 404

def test_aws_resource_types(test_db):
    response = client.get("/api/inventory/aws/accounts/aws-1/regions/us-east-1/types")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]["resource_type"] == "t3"
    assert data[0]["resource_count"] == 1

def test_aws_resources(test_db):
    response = client.get("/api/inventory/aws/accounts/aws-1/regions/us-east-1/resources")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]["resource_id"] == "id4"

def test_aws_resources_with_type_filter(test_db):
    response = client.get("/api/inventory/aws/accounts/aws-1/regions/us-east-1/resources?type=t3")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1

    response_empty = client.get("/api/inventory/aws/accounts/aws-1/regions/us-east-1/resources?type=t99")
    assert response_empty.status_code == 200
    assert len(response_empty.json()) == 0
