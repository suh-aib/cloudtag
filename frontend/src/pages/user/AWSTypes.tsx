    ...
    <div className="space-y-6 max-w-6xl mx-auto py-2">
      ...
      <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-sm mb-6 flex flex-col sm:flex-row justify-between gap-4 items-start sm:items-center">
        ...
        <div className="flex items-center gap-4 shrink-0">
          <label className="flex items-center gap-2 text-sm text-gray-700 font-medium cursor-pointer">
            <input 
              type="checkbox" 
              className="rounded border-gray-300 text-aws focus:ring-aws cursor-pointer"
          ...
          {selectedIds.size > 0 && (
            <div className="flex items-center gap-3 bg-aws-light/30 px-3 py-1 rounded-md border border-aws/20">
              ...
              {!assignUserId && (
                <Button 
                  size="sm" 
                  className="h-7 px-3 gap-1.5 bg-aws hover:bg-aws-dark text-white text-xs" 
                  onClick={() => navigate(`/bulk-tagging/wizard?provider=AWS&scopeType=RESOURCE_TYPE&accountId=${encodeURIComponent(accountId || '')}&region=${encodeURIComponent(region || '')}&resourceType=${encodeURIComponent(Array.from(selectedIds).join(','))}${taskId ? `&task_id=${taskId}` : ''}`)}
                >
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {types.map((type) => (
            <Card key={type.resource_type} className={`border-gray-200 hover:shadow-md transition-shadow group relative ${selectedIds.has(type.resource_type) ? 'ring-2 ring-aws bg-aws-light/5' : ''}`}>
              <div className="absolute top-4 right-4 z-10" onClick={(e) => e.stopPropagation()}>
                <input 
                  type="checkbox" 
                  className="rounded border-gray-300 text-aws focus:ring-aws h-4 w-4 bg-white cursor-pointer"
